...
public class App {
	...
	public static void main(String[] args) {
		...
		Spark.get("/users/:id", (request, response) -> {
			...
			try {
				/* Se intenta parsear el indetificador del recurso como entero */
				...
				/*
				 * Si el identificador no es un entero se asume que el recurso
				 * no existe
				 */
				response.status(404);
			}
		...
		Spark.put("/users/:id", (request, response) -> {
			...
			Object body = new String();
			try {
				/* Se obtiene el valor entero del indentificador del recurso. */
				Integer id = Integer.parseInt(request.params(":id"));
				...
				if (users.get(id) == null) {
					...
					/* En caso que el recurso exista se actualizan sus valores. */
					users.put(id, user);
					/*
					 * Se asigna el usuario a la respuesta que se va a dar en el
					 * cuerpo del mensaje.
					 */
					body = user;
				}
				...
				/*
				 * En caso de no poder parsear el contenido del mensaje en json,
				 * se responde con el error 422 indicando que hay errores
				 * semánticos.
				 */
				response.status(422);
			} catch (NumberFormatException e) {
		...
		Spark.delete("/users/:id", (request, response) -> {
			...
			try {
				/* Se obtiene el valor entero del identificador del recurso. */
				Integer id = Integer.parseInt(request.params(":id"));

				/* Se verifica si el recurso existe. */
				if (users.get(id) == null) {
					/* El recurso no existe y se responde con el código 404. */
					response.status(404);
				} else {
				...
				/*
				 * En caso de no poder parsea el identificador a entero se
				 * responde 404.
				 */
				response.status(404);
			}
